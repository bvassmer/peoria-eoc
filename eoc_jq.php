...
if ($action == 0) {
	...
	echo "IconFile: 1, 19, 17, 15, 15, \"http://www.rawoto.com/images/car.png\"\n";
	...
	$xml_output .= "<spotters>\n";
	$xml_output .= "<timestamp>" . date("F j, Y, g:i:s a") . "</timestamp>";
	
	while($row = mysql_fetch_array( $spotters )) {
		$xml_output .= "\t<spotter>\n";
	    $xml_output .= "\t\t<spottersid>" . $row['spottersid'] . "</spottersid>\n";
        $xml_output .= "\t\t<lat>" . $row['lat'] . "</lat>\n";
        $xml_output .= "\t\t<long>" . $row['long'] . "</long>\n";
